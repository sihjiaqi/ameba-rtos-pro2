...
#if defined(CONFIG_PLATFORM_8735B)
#undef fprintf
#define fprintf fprintf_iperf3
int fprintf_iperf3(FILE *stream, const char *format, ...)
{
	va_list args;
...
iperf_init_test(struct iperf_test *test)
{
	...
	/* Init each stream. */
#if defined(CONFIG_PLATFORM_8735B)
	if (gettimeofday_iperf3(&now, NULL) < 0) {
#else
...
#if defined(CONFIG_PLATFORM_8735B)
		gettimeofday_iperf3(&rp->end_time, NULL);
#else
