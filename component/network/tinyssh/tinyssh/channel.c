...
extern _mutex tinyssh_channel_mutex;
...
int channel_open(const char *user, crypto_uint32 id, crypto_uint32 remotewindow, crypto_uint32 maxpacket, crypto_uint32 *localwindow)
{
	...
	/* copy PATH */
	if (!newenv_copyenv("PATH")) if (!newenv_env("PATH", "/bin:/usr/bin")) {
			return 0;
		}
...
int channel_exec(const char *cmd)
{
	...
	if (channel.flagterminal) {
		extern uint8_t tinyssh_stdio_used;
		tinyssh_stdio_used = 1;
	}
	channel.len0 = 0;
	newenv_purge();
#endif
...
int channel_getfd0(void)
{
	return channel.fd0;
}
int channel_getfd1(void)
{
	return channel.fd1;
}
int channel_getfd2(void)
{
	return channel.fd2;
}
long long channel_getlen0(void)
{
	return channel.len0;
}
crypto_uint32 channel_getid(void)
{
	return channel.id;
}
crypto_uint32 channel_getlocalwindow(void)
{
	return channel.localwindow;
}
