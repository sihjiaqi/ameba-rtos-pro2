...
extern uint8_t tinyssh_stopping;
...
//        if (geteuid() == 0) die_fatal("rejecting to run under UID=0", 0, 0);
	...
	for (i = 0; sshcrypto_keys[i].name; ++i) {
		sshcrypto_keys[i].sign_flagserver |= sshcrypto_keys[i].cryptotype & cryptotypeselected;
	}
	for (i = 0; sshcrypto_keys[i].name; ++i) {
		sshcrypto_keys[i].sign_flagclient |= sshcrypto_keys[i].cryptotype & cryptotypeselected;
	}
...
//    alarm(3600);
	...
	for (;;) {
		...
		if (poll(p, q - p, 60000) < 0) {
			...
			if (watch0) if (!watch0->revents) {
					watch0 = 0;
				}
			if (watch1) if (!watch1->revents) {
					watch1 = 0;
				}
			if (watchfromchild1) if (!watchfromchild1->revents) {
					watchfromchild1 = 0;
				}
			if (watchfromchild2) if (!watchfromchild2->revents) {
					watchfromchild2 = 0;
				}
			if (watchtochild) if (!watchtochild->revents) {
					watchtochild = 0;
				}
			if (watchselfpipe) if (!watchselfpipe->revents) {
					watchselfpipe = 0;
				}
		}
		...
		/* check child */
		if (channel_iseof()) {
			if (selfpipe[0] == -1) if (open_pipe(selfpipe) == -1) {
		...
		extern int tinyssh_check_recv(void);
		int r = tinyssh_check_recv();
		if (r == 1) {
			/* receive data from network */
			if (!packet_recv()) {
				die_fatal("unable to receive data from network", 0, 0);
			...
			die_fatal("error to check receive event from network", 0, 0);
		}
#endif
		/* process packets */
		for (;;) {
