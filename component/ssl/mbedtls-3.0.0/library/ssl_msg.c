...
extern mbedtls_ssl_context *mbedtls_get_ssl_resume_context(void);
static uint8_t ssl_resume_fix_ctr = 0;
static uint8_t ssl_resume_fix_ctr_try = 0;
static uint8_t ssl_resume_fix_ctr_max_try = 0;
static mbedtls_ssl_context *ssl_resume_fix_context = NULL;

void mbedtls_set_ssl_resume_fix_context(mbedtls_ssl_context *ssl)
{
    ssl_resume_fix_context = ssl;
}

void mbedtls_set_ssl_resume_fix_ctr(uint8_t fix_ctr, uint8_t max_try)
{
    ssl_resume_fix_ctr = fix_ctr;
    ssl_resume_fix_ctr_max_try = max_try;
}
...
etm_mac_retry:
                                          ...
                                          transform->maclen ) != 0 )
            {
...
#if defined(CONFIG_SSL_RESUME) && (CONFIG_SSL_RESUME == 1)
                if ( ssl_resume_fix_ctr && ( ssl_resume_fix_ctr_try < ssl_resume_fix_ctr_max_try ) && ssl_resume_fix_context && ( ssl_resume_fix_context->transform_in == transform ) )
                {
                    ssl_resume_fix_ctr_try ++;
                    for ( int i = 8; i > 0; i -- )
                    {
                        if ( ++ add_data[i - 1] != 0 )
                        {
                ...
                else if ( ssl_resume_fix_ctr && ( ssl_resume_fix_ctr_try == ssl_resume_fix_ctr_max_try ) && ssl_resume_fix_context && ( ssl_resume_fix_context->transform_in == transform ) )
                {
                    ssl_resume_fix_ctr = 0;
                }
#endif /* CONFIG_SSL_RESUME */
...
#if defined(CONFIG_SSL_RESUME) && (CONFIG_SSL_RESUME == 1)
            else if ( ssl_resume_fix_ctr && ssl_resume_fix_context && ( ssl_resume_fix_context->transform_in == transform ) )
            {
                ssl_resume_fix_ctr = 0;
                if ( ssl_resume_fix_ctr_try > 0)
                {
                    memcpy(ssl_resume_fix_context->in_ctr, add_data, 8);
                }
            }
#endif /* CONFIG_SSL_RESUME */
