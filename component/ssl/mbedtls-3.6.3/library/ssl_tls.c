...
{
	extern void mbedtls_set_ssl_resume_fix_context(mbedtls_ssl_context *ssl);
	mbedtls_set_ssl_resume_fix_context(ssl);
	...
	if (cipher_info == NULL || md_info == NULL) {
		return -1;
	}
	...
	ssl->transform->tls_version = retention_ssl.transform.tls_version;
	...
	mbedtls_cipher_setkey(&ssl->transform->cipher_ctx_enc, retention_ssl.cipher_key_enc, (int) mbedtls_cipher_info_get_key_bitlen(cipher_info), MBEDTLS_ENCRYPT);
	mbedtls_cipher_setkey(&ssl->transform->cipher_ctx_dec, retention_ssl.cipher_key_dec, (int) mbedtls_cipher_info_get_key_bitlen(cipher_info), MBEDTLS_DECRYPT);
	if (cipher_info->mode == MBEDTLS_MODE_CBC) {
	...
	// ssl context
	ssl->state = MBEDTLS_SSL_HANDSHAKE_OVER;
	ssl->tls_version = retention_ssl.ssl.tls_version;
	ssl->badmac_seen_or_in_hsfraglen = retention_ssl.ssl.badmac_seen_or_in_hsfraglen;
